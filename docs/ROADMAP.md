...
- [x] `lib/auth.ts` — HMAC-signed cookie session (sign, verify, getSession)
...
- [x] `app/layout.tsx` — Updated metadata (site name, description, openGraph), fonts + Toaster + theme
...
- [x] `components/layout/navbar.tsx` — Public navbar (logo, nav links, theme toggle)
...
- [x] `lib/rate-limiter.ts` — In-memory rate limiter
- [x] `lib/r2.ts` — Cloudflare R2 S3 client
- [x] `lib/email.ts` — Nodemailer transport
- [x] `lib/validations/` — Zod schemas for all 5 models
- [x] `lib/data.ts` — Public data-fetching helpers (getPublishedPosts, getActiveServices, etc.)
- [x] `app/admin/(dashboard)/page.tsx` — Dashboard (counts: services, projects, products, posts, unread messages)
- [x] `app/(public)/page.tsx` — Minimal homepage placeholder
- [x] `.env.example` file with all 14 variables (no real values)
...
- [x] `app/api/upload/route.ts` — Server-side multipart upload to R2, return public URL
- [x] `app/admin/_components/upload-button.tsx` — Upload button + preview component
- [x] `app/admin/_components/tip-tap-editor.tsx` — TipTap rich text editor
...
- [x] `app/admin/_actions/posts.ts` — CRUD actions for posts (publish, draft, delete, save TipTap JSON to R2)
...
- [x] `components/sections/contact.tsx` — Contact form (name, email, message)
- [x] `components/shared/scroll-reveal.tsx` — Scroll reveal wrapper (created early for Phase 4)

### Homepage

- [x] `app/(public)/page.tsx` — Compose all sections in order, with section IDs for smooth scroll
...
- [x] `app/(public)/blog/[slug]/page.tsx` — Blog post detail page with JSON-LD
- [x] `app/(public)/blog/_components/tip-tap-content.tsx` — TipTap JSON → HTML renderer
...
- [x] `app/(public)/robots.ts` — Auto-generated robots.txt
...
> SEO metadata, structured data, scroll animations, loading/error states, responsive polish.
