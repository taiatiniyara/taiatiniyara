...
function Index() {
  ...
  const projects = [
    ...
    {
      title: 'Totolaw',
      url: 'https://totolaw.org',
      status: 'Ongoing'
    },
    {
      title: 'Pacific Power Association Performance Benchmarking',
      url: 'http://prismdashboard.org/',
      status: 'Completed'
    },
    {
      title: 'Samoa Tourism Product Database',
      url: 'https://samoatourism-productdb.web.app/login',
      status: 'Completed'
    },
    {
      title: "Niucut",
      url: 'https://niucut.com',
      status: 'Ongoing'
    }
  ]
  ...
  return (
    <>
      <SEO
        title="Taia Tiniyara"
        description="Full-Stack Software Developer specializing in systems architecture, database design, and SaaS development. Expertise in C#, JavaScript, TypeScript, Python, React, Next.js, and more."
      />
      <StructuredData
        type="Person"
        data={{
      ...
      <section className="relative container mx-auto px-4 py-16 pb-24">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-slate-600 dark:text-slate-400">Recent work and ongoing initiatives</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {projects.map((project, index) => {
              return (
                <Card key={project.title} className={`group relative p-6 hover:shadow-xl transition-all duration-300 hover:-translate-y-2 overflow-hidden ${index % 2 === 0 ? 'bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800' : 'bg-slate-100 dark:bg-slate-900/20 border-slate-200 dark:border-slate-700'}`}>
                  ...
                  <div className="relative space-y-4">
                    <div className="flex items-start justify-between gap-4">
                      <h3 className="text-xl font-semibold text-slate-900 dark:text-slate-100 group-hover:text-blue-700 dark:group-hover:text-blue-400 transition-colors">
                        {project.title}
                      </h3>
                      <Badge className={project.status === 'Completed' ? 'bg-green-500 hover:bg-green-600 text-white shadow-md' : 'bg-blue-500 hover:bg-blue-600 text-white shadow-md'}>
                        {project.status}
                      </Badge>
                    </div>
                    <Button 
